...
| --- | --- | --- | --- |
| `src/vs/base/parts/sandbox/test/electron-browser/globals.test.ts` | `test/unit/tauri/desktopSandbox.test.js` | Ported | Validates `window.vscode` bridge shape, process/webFrame/webUtils shims, process event helpers, and global alias wiring. |
...
| `src/vs/base/parts/ipc/test/electron-browser/ipc.mp.test.ts` | `apps/tauri/src-tauri/src/router.rs` + `apps/tauri/ui/src/desktopSandbox.ts` + `test/unit/tauri/desktopChannels.test.js` | In Progress | Added `extensionHostStarter` message-port/event normalization, watcher correlation/stop/verbose behavior, webview event normalization, native-host pick/open navigation flows, broader nativeHost/userDataSync/mcp/extensions/debug event normalization, and listener-failure noop behavior; full transport parity still pending. |
