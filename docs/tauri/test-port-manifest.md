...
| `src/vs/workbench/services/dialogs/test/electron-browser/fileDialogService.test.ts` | `apps/tauri/src-tauri/src/capabilities/dialogs.rs` tests + `apps/tauri/src-tauri/src/router.rs` nativeHost dialog tests + `test/unit/tauri/desktopChannels.test.js` | In Progress | Added method-shape coverage for `showMessage` defaults, dialog helper/validation contracts, nativeHost proxy mapping for `showMessageBox`/`showOpenDialog`/`showSaveDialog` (including defaults, folder-vs-file routing, and cancellation/path shaping), and desktop-channel normalization for direct nativeHost dialog result shapes; interactive picker semantics still expanding. |
