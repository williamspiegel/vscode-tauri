...
| `src/vs/base/parts/ipc/test/electron-browser/ipc.mp.test.ts` | `apps/tauri/src-tauri/src/router.rs` + `apps/tauri/ui/src/desktopSandbox.ts` + `test/unit/tauri/desktopChannels.test.js` + `test/unit/tauri/hostClient.test.js` | In Progress | Added `extensionHostStarter` message-port/event normalization, extension-host message-port bridge roundtrip and close-event coverage (including manual acquired-port close cleanup back into the host bridge), renderer channel server bootstrap/message forwarding via `vscode:hello` and `vscode:message`, watcher correlation/stop/verbose behavior, watcher invalid-request validation, webview event normalization, native-host pick/open navigation flows, broader nativeHost/userDataSync/mcp/extensions/debug/watcher event normalization, listener-failure noop behavior, direct `HostClient` transport contracts (handshake request shape, unknown-method short-circuiting, transport error wrapping, malformed envelope handling, window-config cache retry, buffered desktop channel events, stable invalid payload errors, and `__TAURI_INTERNALS__` event-listen fallback), and helper edge-case contracts (missing watch-id arg validation, nested byte payload decode normalization, workspace key `None` behavior, fallback-metrics numeric filtering, pre-epoch timestamp fallback); full transport parity still pending. |
...
| --- | --- | --- | --- |
| API tests (folder/workspace) | `scripts/test-tauri-integration.sh` | In Progress | Executable path wired, and the `code-tauri` launcher now forwards workspace/extension CLI args into `cargo tauri`; opt-in execution via `VSCODE_TAURI_RUN_API_INTEGRATION=1`. |
