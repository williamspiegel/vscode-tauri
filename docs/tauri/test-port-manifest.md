...
| `src/vs/base/parts/ipc/test/electron-browser/ipc.mp.test.ts` | `apps/tauri/src-tauri/src/router.rs` + `apps/tauri/ui/src/desktopSandbox.ts` + `test/unit/tauri/desktopChannels.test.js` | In Progress | Added `extensionHostStarter` message-port/event normalization, watcher correlation/stop/verbose behavior, webview event normalization, and listener-failure noop behavior; full transport parity still pending. |
