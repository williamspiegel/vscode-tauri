...
| `src/vs/workbench/services/dialogs/test/electron-browser/fileDialogService.test.ts` | `apps/tauri/src-tauri/src/capabilities/dialogs.rs` tests + `apps/tauri/src-tauri/src/router.rs` nativeHost dialog tests | In Progress | Added method-shape coverage for `showMessage` defaults, dialog helper/validation contracts, and nativeHost proxy mapping for `showMessageBox`/`showOpenDialog`/`showSaveDialog` (including defaults, folder-vs-file routing, and cancellation/path shaping); interactive picker semantics still expanding. |
