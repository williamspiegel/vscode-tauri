...
import { app, dialog } from 'electrobun';
...
class CodeMain {
	...
	private createServices(): [
		...
		if (isWindows && productService.win32RegValueName) {
			policyService = disposables.add(
				new NativePolicyService(logService, productService.win32RegValueName),
			);
			...
			policyService = disposables.add(
				new FilePolicyService(
					URI.file(LINUX_SYSTEM_POLICY_FILE_PATH),
					fileService,
					logService,
				),
			);
	...
	private async initServices(
		...
		console.log('--- inside initServices() ---');
		await Promises.settled<unknown>([
			...
			Promise.all<string | undefined>(
				[
					...
					environmentMainService.localHistoryHome.with({ scheme: Schemas.file })
						.fsPath,
					environmentMainService.backupHome,
				].map((path) =>
					path ? promises.mkdir(path, { recursive: true }) : undefined,
				),
			).then(() => console.log('--- mkdir done ---')),

			// State service
			stateService
				.init()
				.then(() => console.log('--- stateService.init() done ---')),

			// Configuration service
			configurationService
				.initialize()
				.then(() =>
					console.log('--- configurationService.initialize() done ---'),
				),
		]);
	...
	private async windowsAllowSetForegroundWindow(
		...
		if (isWindows) {
			...
			try {
				(await import('windows-foreground-love')).allowSetForegroundWindow(
					processId,
				);
			} catch (error) {
	...
	private async checkInnoSetupMutex(
		productService: IProductService,
	): Promise<boolean> {
		if (
			!(
				isWindows &&
				productService.win32MutexName &&
				productService.win32VersionedUpdate
			)
		) {
			return false;
		}
